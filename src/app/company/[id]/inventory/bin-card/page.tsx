...
function computeBinCard(entries: JournalEntry[], itemName: string): BinCardRow[] {
  ...
  for (const entry of entries) {
    for (const line of entry.lines) {
      ...
      const received = line.debit || 0;
      const issued = line.credit || 0;
...
export default function BinCardPage() {
  ...
  const binCard = useMemo(() => selectedItem ? computeBinCard(entries, selectedItem) : [], [entries, selectedItem]);
  ...
  const columns = [
    ...
    { header: 'Issued (₹)', key: 'issued', align: 'right' as const, isMono: true },
    { header: 'Balance (₹)', key: 'balance', align: 'right' as const, isMono: true },
  ];

  return (
    <div>
      <PageHeader title="Bin Card" description="Quantity-wise stock movement for individual items">
        <div className="flex flex-col gap-2 items-end">
          <DateRangeFilter fromDate={fromDate} toDate={toDate} onDateChange={(f, t) => { setFromDate(f); setToDate(t); }} />
          {binCard.length > 0 && (
            <ExportButtons title={`Bin Card - ${selectedItem}`} companyName={company.name} entityType={entityLabel} dateRange={`${fromDate} to ${toDate}`} columns={columns} data={binCard} />
          )}
        </div>
      </PageHeader>
      ...
      <div className="mb-4 bg-white border border-gray-200 rounded-xl px-4 py-3">
        ...
        {loading ? (
          ...
          <select
            ...
            className="w-full max-w-md px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
          >
      ...
      {selectedItem && binCard.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
          <div className="text-center py-3 border-b border-gray-200 bg-gray-50/50">
            ...
            <h3 className="text-base font-bold text-gray-900 mt-0.5">Bin Card — {selectedItem}</h3>
              ...
              <tbody>
                {binCard.map((r, i) => (
                  <tr key={`${r.date}-${i}`} className="border-b border-gray-100">
                    ...
                    <td className="px-3 py-2 text-right font-mono text-[13px] tabular-nums">{r.received > 0 ? formatIndianCurrency(r.received) : ''}</td>
                    <td className="px-3 py-2 text-right font-mono text-[13px] tabular-nums">{r.issued > 0 ? formatIndianCurrency(r.issued) : ''}</td>
                    <td className="px-3 py-2 text-right font-mono text-[13px] tabular-nums font-semibold">{formatIndianCurrency(r.balance)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
